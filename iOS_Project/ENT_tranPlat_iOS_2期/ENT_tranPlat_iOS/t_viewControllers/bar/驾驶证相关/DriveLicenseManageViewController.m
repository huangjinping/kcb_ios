...
- (void)reloadViews
{
    ...
    _weifaCountL = [[UILabel alloc] initWithFrame:YYRectMake(label.r - 100, 24, 70, 50)];
    ...
    [_enterWeifaButton setFrame:YYRectMake(label.r + 50, label.t + 3, 30*4, 30)];
    _enterWeifaButton.tag = BUTTON_TAG_ENTER_LIST;
    [bgView addSubview:_enterWeifaButton];
    
    UIImageView *yanzhengImgView = [[UIImageView alloc] initWithFrame:YYRectMake(bgView.w - 35 - 30, 30, 35, 35)];
    ...
    UILabel *contentL = [[UILabel alloc] initWithFrame:YYRectMake(label.r + 30, label.t, bgView.w - 30 - label.r - 30, label.h)];
    ...
    label = [[UILabel alloc] initWithFrame:YYRectMake(30, lineHeightPX + 30, 100, 30)];
    [label convertNewLabelWithFont:FONT_NOMAL textColor:COLOR_FONT_NOMAL textAlignment:NSTextAlignmentLeft];
    label.text = @"证件号码";
    size = [label.text sizeWithFont:label.font constrainedToSize:CGSizeMake(1000, 30)];
    label.size = size;
    ...
    contentL = [[UILabel alloc] initWithFrame:YYRectMake(label.r + 30, label.t, bgView.w - 30 - label.r - 30, label.h)];
    [contentL convertNewLabelWithFont:FONT_NOMAL textColor:COLOR_FONT_INFO_SHOW textAlignment:NSTextAlignmentLeft];
    ...
    label = [[UILabel alloc] initWithFrame:YYRectMake(30, lineHeightPX*2 + 30, 100, 30)];
    [label convertNewLabelWithFont:FONT_NOMAL textColor:COLOR_FONT_NOMAL textAlignment:NSTextAlignmentLeft];
    label.text = @"准驾车型";
    size = [label.text sizeWithFont:label.font constrainedToSize:CGSizeMake(1000, 30)];
    label.size = size;
    ...
    label = [[UILabel alloc] initWithFrame:YYRectMake(30, lineHeightPX*3 + 30, 100, 30)];
    [label convertNewLabelWithFont:FONT_NOMAL textColor:COLOR_FONT_NOMAL textAlignment:NSTextAlignmentLeft];
    label.text = @"证芯编号";
    size = [label.text sizeWithFont:label.font constrainedToSize:CGSizeMake(1000, 30)];
    label.size = size;
...
- (void)reloadWeifaCount{
    ...
    if (range.location != NSNotFound) {
        ...
        if (_netStatus == loadingPeccancyRecordFailed) {
            ...
            if (record) {//显示记录数
                _weifaCountL.text = [NSString stringWithFormat:@"%d", [[Helper drivePeccancyMsgAnalysis:record.jszwf_detail] count]];
            }
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [_editButton.titleLabel setFont:FONT_NOMAL];
    [_editButton setFrame:YYRectMake(_navigationImgView.w - 100, (_navigationImgView.h - 30)/2, 60, 30)];
    [_editButton addTarget:self action:@selector(editButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
    [_navigationImgView addSubview:_editButton];
...
- (void)editButtonClicked:(UIButton*)b{
