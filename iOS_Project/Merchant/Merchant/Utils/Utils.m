...
//  Copyright (c) 2014年 bbtree. All rights reserved.
//
...
+ (NSString *)equipment
{
    ...
    NSString *udid = [SSKeychain passwordForService:bundleName account:@"deviceid"];

    if (udid == 0) {
        NSString *uuidStr = [[[UIDevice currentDevice] identifierForVendor] UUIDString];

        [SSKeychain setPassword:[NSString stringWithFormat:@"%@", uuidStr]
                     forService:bundleName
                        account:@"deviceid"];
...
UIImage * ImageByScalingAndCroppingForSize(UIImage *sourceImage, CGSize targetSize)
{
    ...
    if (h <= targetSize.height && w <= targetSize.width) {
        ...
        float b = (float)targetSize.width / w;
...
+ (CGFloat)getHeightByTextContent:(NSString *)text fontSize:(CGFloat)size space:(CGFloat)space
{
    NSMutableParagraphStyle *paragraphStyle = [[NSMutableParagraphStyle alloc] init];
    
//    paragraphStyle.lineSpacing = size;
    CGSize titleSize = [text boundingRectWithSize:CGSizeMake(APP_WIDTH - 2 * space, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{ NSFontAttributeName: [UIFont systemFontOfSize:size], NSParagraphStyleAttributeName: paragraphStyle } context:nil].size;
...
/*
 type: 1:发货状态 2、订单状态
 */
+ (NSString *)getStatusNameType:(NSInteger)type statusId:(NSString *)ids{
    if (type == 1) {
        NSDictionary *dataSourceDict = @{@"1":@"待确认",@"2":@"待发货",@"3":@"待收货",@"4":@"已收货"};
        return dataSourceDict[ids];
    }else if(type == 2){
        NSMutableDictionary *dict = [NSMutableDictionary dictionary];
        for (OrderStatus *item in ApplicationDelegate.shareLoginData.userdata.order) {
            [dict setObject:item.value forKey:item.status];
        }
        return dict[ids];
    }
    return nil;
...
+ (BOOL)saveToArchiverWithObject:(id)object toDocumentWithFileName:(NSString *)fileName
{
    NSString *filePath = [[self getDocumentPath] stringByAppendingPathComponent:fileName];
...
+ (id)readArchiverDocumentWithFileName:(NSString *)fileName
{
    NSString *filePath = [[self getDocumentPath] stringByAppendingPathComponent:fileName];
