...
#define playerBounds (CGRectMake(0, APP_Y, APP_WIDTH, 180))
- (void)loadView
{
    // LoggerStream(1, @"loadView");
    //CGRect bounds = [[UIScreen mainScreen] applicationFrame];
    //CGRect bounds = CGRectMake(APP_X, APP_VIEW_Y, APP_WIDTH, 150);
    CGRect bounds = playerBounds;

    self.view = [[UIView alloc] initWithFrame:playerBounds];
    ...
    _gobackPageButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _gobackPageButton.frame = CGRectMake(10, 2, topH - 2*2, topH - 2*2);
    _gobackPageButton.showsTouchWhenHighlighted = YES;
    [_gobackPageButton setBackgroundImage:[UIImage imageNamed:@"go_back.png"] forState:UIControlStateNormal];
    _gobackPageButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
    [_gobackPageButton addTarget:self action:@selector(gobackPageDidTouch:) forControlEvents:UIControlEventTouchUpInside];
    [_topBar addSubview:_gobackPageButton];

    _zoomButton = [UIButton buttonWithType:UIButtonTypeCustom];
    _zoomButton.frame = CGRectMake(width-topH - 10, 3, topH - 3*2, topH - 3*2);
    _zoomButton.showsTouchWhenHighlighted = YES;
    [_zoomButton setBackgroundImage:[UIImage imageNamed:@"movie_screen_size"] forState:UIControlStateNormal];
    _zoomButton.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin;
    [_zoomButton addTarget:self action:@selector(zoomDidTouch:) forControlEvents:UIControlEventTouchUpInside];
    [_topBar addSubview:_zoomButton];
    
    
    _bottomBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, height-botH, width, botH)];
    [_bottomBar setBarStyle:UIBarStyleBlack];
    ...
    _topHUD.frame = CGRectMake(0,height - botH,width,_topBar.frame.size.height);
    _topHUD.autoresizingMask = UIViewAutoresizingFlexibleWidth;

    
    
    
    _progressLabel = [[UILabel alloc] initWithFrame:CGRectMake(25, 1, 50, topH)];
    ...
    _progressSlider = [[UISlider alloc] initWithFrame:CGRectMake(_progressLabel.right, 5, 205, 20)];
    _progressSlider.autoresizingMask = UIViewAutoresizingFlexibleWidth;
    _progressSlider.continuous = NO;
    _progressSlider.value = 0;
    
    _leftLabel = [[UILabel alloc] initWithFrame:CGRectMake(_progressSlider.right, 1, 60, topH)];
    ...
    [_playBtn setBackgroundImage:[UIImage imageNamed:@"movie_play"] forState:UIControlStateNormal];
    [_playBtn setFrame:CGRectMake(10, (topH-30)/2+1, 30, 30)];
...
- (void)gobackPageDidTouch:(id)sender{
    for (UIView *view = self.view.superview; view ; view = view.superview) {
        if ([view.nextResponder isKindOfClass:[BasicViewController class]]) {
            BasicViewController *con = (BasicViewController*)view.nextResponder;
            [con gobackPage];
        }
    }
}

- (void) zoomDidTouch: (id) sender
{
    UIButton *btn = (UIButton*)sender;
    if (_fullscreen) {
        [btn setBackgroundImage:[UIImage imageNamed:@"movie_screen_size"] forState:UIControlStateNormal];
        [self  fullscreenMode:NO];
    }else{
        [btn setBackgroundImage:[UIImage imageNamed:@"movie_screen_fit"] forState:UIControlStateNormal];
...
- (void) setupPresentView
{
    ...
    if (_decoder.validVideo) {
        ...
        _imageView.image = [UIImage imageNamed:@"bg_placeholder_img280*90.gif"];
...
{
    if (self.playing) {
        [_playBtn setBackgroundImage:[UIImage imageNamed:@"movie_pause"] forState:UIControlStateNormal];
    }else{
        [_playBtn setBackgroundImage:[UIImage imageNamed:@"movie_play"] forState:UIControlStateNormal];
    }
}
...
- (void) showHUD: (BOOL) show
{
    ...
    [UIView animateWithDuration:0.2
                     ...
                     animations:^{
                         ...
                         CGFloat alpha = _hiddenHUD ? 0 : 0.7;
                         _topBar.alpha = alpha;
                         _topHUD.alpha = alpha;
...
#if 1
    _fullscreen = on;
    CGFloat animateTime = 0.3f;
    if (on){
        _gobackPageButton.hidden = YES;
        [UIView animateWithDuration:animateTime animations:^{

            [[UIApplication sharedApplication] setStatusBarHidden:YES];
            [self.view setTransform: CGAffineTransformMakeRotation(M_PI_2)];
            [self.view setFrame:CGRectMake(0, 0, APP_WIDTH,[[UIScreen mainScreen] bounds].size.height)];
            if (!iOS7) {
                [self.view setFrame:CGRectMake(0, -20, APP_WIDTH,[[UIScreen mainScreen] bounds].size.height)];
            }
        }];
    
    }else{
        _gobackPageButton.hidden = NO;
        [UIView animateWithDuration:animateTime animations:^{

            [[UIApplication sharedApplication] setStatusBarHidden:NO];
            [self.view setTransform: CGAffineTransformMakeRotation(0)];
            [self.view setFrame:playerBounds];
        }];
