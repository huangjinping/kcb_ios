...
- (instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _rBtn.frame = CGRectMake(self.width/2, 0, 204*x_6_plus, self.height-4*y_6_plus);
        [_rBtn setTitle:@"订单详情"];
        ...
        _line = [[UIView alloc]initWithFrame:CGRectMake(_lBtn.x, _lBtn.bottom, _lBtn.width, 4*y_6_plus)];
...
- (void)viewDidLoad {
    ...
    __block __typeof(rVC) weakRVC = rVC;
    __block __typeof(lVC) weakLVC = lVC;
    __block __typeof(self) weakSelf = self;
    lVC.commplete = ^(NSMutableDictionary *dataSource){
        weakRVC.dataSource = dataSource;
    };
    ...
    cv.block = ^(NSInteger n){
        if (n == 1) {
            [UIView animateWithDuration:.2f animations:^{
                ...
                weakSelf.scrollView.contentOffset = CGPointMake(0, point.y);
            }];
            
            [weakLVC viewWillAppear:NO];
            [weakRVC viewWillDisappear:NO];
        }else{
            [UIView animateWithDuration:.2f animations:^{
                CGPoint point = weakSelf.scrollView.contentOffset;
                weakSelf.scrollView.contentOffset = CGPointMake(weakSelf.scrollView.width, point.y);
            }];
            
            [weakLVC viewWillDisappear:NO];
            [weakRVC viewWillAppear:NO];
        }
    };
