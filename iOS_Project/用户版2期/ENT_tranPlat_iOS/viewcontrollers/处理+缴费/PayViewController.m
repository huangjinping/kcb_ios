...
//    NSString * encodeUrlStr = CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes( kCFAllocatorDefault, (CFStringRef)urlStr, NULL, NULL,  kCFStringEncodingUTF8));
    ...
    NSString *dizhi = @"";
    if (ENT_DEBUG){
        dizhi = @"new.956122.com";
    }else{
        dizhi = @"yetapi.956122.com";
    }
   // NSString *urlStr = [NSString stringWithFormat:@"http://%@/peccancy.do?action=jdsPay&jdsbh=%@&username=%@", dizhi,  _jdsbh, APP_DELEGATE.userName];
    //测试
   // NSString *urlStr = [NSString stringWithFormat:@"http://132.96.77.198:8081/peccancy.do?action=jdsPay&jdsbh=%@&username=%@",  _jdsbh, APP_DELEGATE.userName];
    //正式
    NSString *urlStr = [NSString stringWithFormat:@"http://www.956122.com/peccancy.do?action=jdsPay&jdsbh=%@&username=%@",  _jdsbh, APP_DELEGATE.userName];
    NSString * encodeUrlStr = CFBridgingRelease(CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (CFStringRef)urlStr, NULL, NULL,  kCFStringEncodingUTF8));
