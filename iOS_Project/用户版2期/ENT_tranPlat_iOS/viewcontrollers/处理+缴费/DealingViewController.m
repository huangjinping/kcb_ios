...
- (void)viewDidAppear:(BOOL)animated{
    ...
    UIView  *buttonBgView = [[UIView alloc] initWithFrame:LGRectMake(0, self.view.height/PX_Y_SCALE - 100, APP_PX_WIDTH, 100)];
...
//    [cancelButton setFrame:LGRectMake(0, 20, width, height)];
//    [buttonBgView addSubview:cancelButton];
    ...
    [submitButton setFrame:LGRectMake(width/2, 20, width, height)];
...
- (IBAction)submit:(UIButton*)btn{
    ...
    _alertWhileSubmiting.userInteractionEnabled = NO;
    ...
    if([self.carPeccancyMsg.cfd isEqualToString:@"云南"] ||[self.carPeccancyMsg.cfd isEqualToString:@"海南"]||[self.carPeccancyMsg.cfd isEqualToString:@"重庆"]||[self.carPeccancyMsg.cfd isEqualToString:@"青海"]){
        if([self.carPeccancyMsg.cfd isEqualToString:@"云南"] ){
    ...
    [[Network sharedNetwork] postBody:bodyDict isResponseJson:YES doShowIndicator:YES callBackWithObj:nil onCompletion:^(PostResult result, id requestObj, NSObject *callBackObj) {
        ...
        if (result == postSucc) {
            ...
            if (code == 0) {//成功
                ...
                NSString *cfd = self.carPeccancyMsg.cfd;
                if ([cfd isEqualToString:@"云南"]) {//succ
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息"
                                                                    message:@"提交成功！请等待查看处理结果。"
                                                                   delegate:self
                                                          cancelButtonTitle:@"确认"
                    ...
                    alert.tag = TAG_ALERT_REQ_SUCC;
                    [alert show];
                }else if ([cfd isEqualToString:@"重庆"] || [cfd isEqualToString:@"青海"] || [cfd isEqualToString:@"海南"]){//fail
                    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息"
                                                                    message:message
                                                                   delegate:self
                                                          cancelButtonTitle:@"确认"
                    ...
                    if ([self.car.hphm hasPrefix:@"云"]) {//succ
                        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息"
                                                                        message:@"提交成功！请等待查看处理结果。"
                                                                       delegate:self
                                                              cancelButtonTitle:@"确认"
