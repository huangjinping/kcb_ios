...
- (void)viewDidLoad {
    ...
    _xslc = self.carInfo.xslc;
    _xslsj = self.carInfo.xslsj;
    _dataSource = @[@[@{@"name":@""},
                      @{@"name":@""}],
                    @[@{@"name":@"当前行驶里程（公里）"},
                      @{@"name":@"新车上路时间"}]];
...
- (UIButton *)footerView{
    ...
    [_footerView addActionBlock:^(id weakSender) {
        ...
        if (![_xslc isLegal]) {
            [UITools alertWithMsg:@"请编辑行驶里程"];
            return ;
        }
        if (![_xslsj isLegal]) {
            [UITools alertWithMsg:@"请编辑新上路时间"];
            return ;
        }
        
        weakSelf.carInfo.xslc = _xslc;
        weakSelf.carInfo.xslsj = _xslsj;
        BOOL res = NO;
        while (!res) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 1) {
        if (indexPath.row == 0) {
            cell.subNameLabel.text = self.carInfo.xslc;
        }else{
            cell.subNameLabel.text = self.carInfo.xslsj;
        }
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0 && indexPath.row == 1) {
        ...
        if (indexPath.section == 1) {
            EidtCarViewController *evc = [[EidtCarViewController alloc] init];
            if (indexPath.row == 0) {
                EdittingCell *cell = (EdittingCell *)[tableView cellForRowAtIndexPath:indexPath];
                evc.title = @"行驶里程";
                evc.text = self.carInfo.xslc;
                evc.block = ^(NSString *str){
                    cell.subNameLabel.text = str;
                    _xslc = str;
                };
            }else{
                EdittingCell *cell = (EdittingCell *)[tableView cellForRowAtIndexPath:indexPath];
                evc.title = @"上路时间";
                evc.text = self.carInfo.xslsj;
                evc.block = ^(NSString *str){
                    cell.subNameLabel.text = str;
                    _xslsj = str;
                };
