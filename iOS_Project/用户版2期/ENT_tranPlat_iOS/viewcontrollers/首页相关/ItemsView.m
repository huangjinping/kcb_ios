...
+ (ItemsView *)shareWithItems:(NSArray *)items
    ...
    selectPic = [NSArray arrayWithArray:selectPic];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.icon setImage:[UIImage imageNamed:pictures[indexPath.row]] forState:UIControlStateNormal];
    [cell.icon setImage:[UIImage imageNamed:selectPic[indexPath.row]] forState:UIControlStateHighlighted];
    cell.l.text = dataSource[indexPath.item];
    cell.tag = indexPath.item;
    cell.commplete = ^(NSInteger item){
        if ([self.delegate respondsToSelector:@selector(ItemsView:didSelectedItemAtPath:)]) {
            [self.delegate ItemsView:self didSelectedItemAtPath:item];
        }
    };
