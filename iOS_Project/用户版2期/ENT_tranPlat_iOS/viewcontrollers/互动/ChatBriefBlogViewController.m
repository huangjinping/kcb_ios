...
- (void)loadView_{
    ...
    _blogTableView = [[UITableView alloc] initWithFrame:CGRectMake(APP_X, 0, APP_WIDTH, APP_HEIGHT - APP_NAV_HEIGHT - APP_TAB_HEIGHT)];
...
//    _blogTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, APP_WIDTH, self.view.bounds.size.height)];
    ...
    //刷新
    [_blogTableView addLegendHeaderWithRefreshingTarget:self refreshingAction:@selector(headerRefreshing)];
...
- (NSMutableArray *)arrayWithUserString:(NSArray *)pUserArray{
    ...
    for (NSString *userString in pUserArray) {
        int lent = (int)[userString length];
        if (lent > 2) {
...
//    }
    [bodyDict setObject:@"0" forKey:@"bsort"];
    if (indexId) {
