...
- (void)_loadView{
    ...
    [_leftSegButton setFrame:CGRectMake(0, APP_VIEW_Y + 2, segWidth, segHeight - 4)];
    ...
    [_rightSegButton setFrame:CGRectMake(segWidth, APP_VIEW_Y + 2, segWidth, segHeight - 4)];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [self setCustomNavigationTitle:_car_.hphm];
...
- (void)checkPingzheng:(UIButton*)button{
    CarPeccancyMsg *carMsg = [_carMsgs_pay objectAtIndex:button.tag];
    //PingzhengViewController *pzVC = [[PingzhengViewController alloc] initWithPeccancymsg:carMsg cphm:_car_.hphm andHpzl:_car_.hpzl];
    PingzhengViewController *pzVC = [[PingzhengViewController alloc]
                                     initWithCljg:carMsg.cljgmc
                                     jdsbh:carMsg.jdsbh
                                     wsjyw:carMsg.wsjyw
                                     dsr:carMsg.dsr
                                     jszh:carMsg.jszh
    ...
    if ([_dealingMsg.hphm_head isEqualToString:@"0"] && [_dealingMsg.qrbj isEqualToString:@"1"]) {
        ...
        if ([_dealingMsg.cfd isEqualToString:@"海南"] || [_dealingMsg.cfd isEqualToString:@"云南"] || [_dealingMsg.cfd isEqualToString:@"青海"] || [_dealingMsg.cfd isEqualToString:@"重庆"]) {
            ...
            if (![markAddr hasPrefix:@"琼"] && ![markAddr hasPrefix:@"云"] && ![markAddr hasPrefix:@"青"] && ![markAddr hasPrefix:@"渝"]) {
                ...
                [UIAlertView alertTitle:@"违法处理" msg:@"此条违法处理机关所在地尚未开通网上处理，暂无法处理"];
        ...
        if (![markAddr hasPrefix:@"云"]) {
            if ([_dealingMsg.wfjfs integerValue] > 0) {
                [UIAlertView alertTitle:@"违法处理" msg:@"当前车辆违法记录有扣分，暂无法处理"];
                return;
            }
        }
        
        //现修所有大型车不可处理
        if ([_car_.hpzl isEqualToString:@"01"]) {
            [UIAlertView alertTitle:@"违法处理" msg:@"当前违法记录为大型车违法，暂无法处理"];
            return;
        }
        ...
        if (![markAddr hasPrefix:@"云"]) {
            ...
            BOOL chuliguo = [[NSUserDefaults standardUserDefaults] boolForKey:[KEY_YUN_NAN_CHU_LI stringByAppendingString:_car_.hphm]];
            if (chuliguo) {
        ...
        [UIAlertView alertTitle:@"违法处理" msg:@"根据是否可处理标记判定此条违法无法处理，详情咨询4000-956122"];
    }
}
...
- (void)alertTFVFinishedEntry:(AlertTextFieldView*)alertTFV{
    ...
    if ([_car_.syr isEqualToString:name] && [_car_.sfzmhm isEqualToString:idnum]) {
        ...
        [[NSUserDefaults standardUserDefaults] setBool:YES forKey:[KEY_YUN_NAN_CHU_LI stringByAppendingString:_car_.hphm]];
        //转到处理页面
        [self dealPeccancyWithCarPeccancyMsg:_dealingMsg isyun:YES];
...
//    @property(nonatomic, retain) IBOutlet UILabel       *cfbzLabelLabel;//处罚标准
    ...
    if (cell == nil) {
        ...
        [cell.checkPingzhengButton addTarget:self action:@selector(checkPingzheng:) forControlEvents:UIControlEventTouchUpInside];
        ...
        cell.checkPingzhengButton.backgroundColor = [UIColor whiteColor];
        [cell.checkPingzhengButton setTitleColor:COLOR_LINK forState:UIControlStateNormal];
    }
    ...
    cell.checkPingzhengButton.tag = indexPath.row;
    ...
    if (_isDealingRecord) {
        ...
        if ([msg.photostatus isEqualToString:@"1"]) {
            ...
            [cell.checkPhotoButton setTitle:@"失败 重新申请" forState:UIControlStateNormal];
   ...
   // cell.cjjgLabelLabel.centerY=cell.cjjgL.centerY;
    [cell.cjjgL setFrame:CGRectMake(cell.cjjgLabelLabel.right, cell.cjjgLabelLabel.top + 1, APP_WIDTH - cell.cjjgLabelLabel.right - space*2, [self heightForStr:cell.cjjgL.text])];
     cell.cjjgL.centerY=cell.cjjgLabelLabel.centerY;
    ...
    if (!_isDealingRecord) {
        ...
        if ([_car_.hphm hasPrefix:@"云"]) {
            [cell.payButton setFrame:CGRectMake(0, buttonY, cell.bgView.width*2/3, BUTTON_HEIGHT)];
            [cell.checkPingzhengButton setFrame:CGRectMake(cell.bgView.width*2/3, buttonY, cell.bgView.width*1/3, BUTTON_HEIGHT)];
            cell.checkPingzhengButton.hidden = NO;
    ...
    if (!_isDealingRecord) {
        if ([_car_.hphm hasPrefix:@"云"]) {
            [cell.lineVertical setHidden:NO];
            [cell.lineVertical setFrame:CGRectMake(cell.payButton.right, cell.line2.top, 1, BUTTON_HEIGHT)];
    ...
    cell.cjjgL.font = WY_FONT_SIZE(38);
    ...
    cell.cjjgLabelLabel.font = WY_FONT_SIZE(38);
    cell.wfsjLabelLabel.font = WY_FONT_SIZE(38);
    cell.wfddLabelLabel.font = WY_FONT_SIZE(38);
    cell.wfxwLabelLabel.font = WY_FONT_SIZE(38);
    cell.cfbzLabelLabel.font = WY_FONT_SIZE(38);
    cell.cjjgL.textColor=[UIColor colorWithHex:0x666666];
    ...
    cell.cfbzLabelLabel.textColor=[UIColor colorWithHex:0x666666];
    cell.cjjgLabelLabel.textColor=[UIColor colorWithHex:0x666666];
    ...
    cell.payButton.titleLabel.font=(WY_FONT_SIZE(38));
    cell.checkPingzhengButton.titleLabel.font=(WY_FONT_SIZE(38));
    cell.checkPhotoButton.titleLabel.font=(WY_FONT_SIZE(38));
    cell.dealPeccancyButton.titleLabel.font=(WY_FONT_SIZE(38));
