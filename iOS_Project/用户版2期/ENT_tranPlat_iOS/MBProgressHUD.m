...
+ (MB_INSTANCETYPE)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ...
    hud.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"loading图"]];
    hud.customView.size = CGSizeMake(200*x_6_plus, 200*x_6_plus);
    hud.removeFromSuperViewOnHide = YES;
    [view addSubview:hud];
    [hud show:animated];
    return MB_AUTORELEASE(hud);
...
- (void)rotation{
    if (customView) {
        ...
        animation.duration = .23f;
        animation.fromValue = [NSNumber numberWithFloat:0.0];
        animation.toValue = [NSNumber numberWithFloat:-60 * M_PI];
