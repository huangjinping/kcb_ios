...
- (void)layoutSubviews
{
    ...
    if ([[self.monthShowing stringWithDateFormat:DateFormatWithYearMonth] isEqualToString:[[NSDate date] stringWithDateFormat:DateFormatWithYearMonth]]) {
...
//            dateButton.layer.borderColor = [UIColor whiteColor].CGColor;
        ...
        if ([[date earlierDate:[NSDate date]] isEqualToDate:date] || [date isEqualToDate:[NSDate date]]) {
...
- (void)moveCalendarToNextMonth
{
    ...
    if ([self.delegate respondsToSelector:@selector(calendar:didMoveToMonth:)]) {
        [self.delegate calendar:self didMoveToMonth:self.monthShowing];
    }
}
