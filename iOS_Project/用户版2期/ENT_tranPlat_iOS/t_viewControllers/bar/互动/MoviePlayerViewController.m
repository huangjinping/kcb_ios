...
- (void)netMovieCommentsWithBlogID:(NSString *)blog_id  indexid:(NSString *)indexid ismore:(BOOL)ismore{
    ...
    [[ChatNetwork sharedChatNetwork] chatPostBody:bodyDict onUrl:@"findComments.do" withPostMethod:@"GET" isResponseJson:YES doShowIndicator:YES onView:_commentTableView callBackWithObj:[NSString stringWithFormat:@"%d",ismore] onCompletion:^(PostResult result, id requestObj, NSObject *callBackObj){
        if (result == postSucc) {
            NSDictionary *resDict = (NSDictionary*)requestObj;
            NSArray *contentsArr = [resDict analysisArrValueByKey:@"contents"];
