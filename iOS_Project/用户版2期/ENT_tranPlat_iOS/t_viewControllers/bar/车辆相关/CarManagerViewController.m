...
- (void)addBlankviewOnView:(UIView*)v{
    UIImageView *carBindView = [[UIImageView alloc] initWithFrame:YYRectMake((v.w - 339)/2, (v.h - 45)/2, 339, 45)];
    carBindView.image = [UIImage imageNamed:@"add_car"];
    carBindView.userInteractionEnabled = YES;
    [v addSubview:carBindView];
    
    UILabel *l = [[UILabel alloc] initWithFrame:YYRectMake(30, v.h + 5, APP_PX_WIDTH - 30,30)];
...
- (void)addCar:(CarInfo*)car carIndex:(NSInteger)index onView:(UIView*)v{
    ...
    UIImageView *imgVv = [[UIImageView alloc] initWithFrame:YYRectMake(30, 30, 140, 140)];
    UIImage *img = [[UIImage imageNamed:@"home_car_brand_bg"] resizableImageWithCapInsets:UIEdgeInsetsMake(14, 14, 14, 14)];
    [imgVv setImage:img];
    [v addSubview:imgVv];
    UIImageView *carLogoView = [[UIImageView alloc] initWithFrame:YYRectMake((imgVv.w - 100*250/200)/2, (imgVv.h - 100)/2, 100*250/200, 100)];
    ...
    UILabel *carNumL = [[UILabel alloc] initWithFrame:YYRectMake(imgVv.r + 30, 50, 0, 40)];//(50, 20, 130, 30)];
    ...
    UILabel *bangdingL = [[UILabel alloc] initWithFrame:YYRectMake(carNumL.r + 30, carNumL.t + 3, 140, 30)];
    ...
    UILabel *timeL = [[UILabel alloc] initWithFrame:YYRectMake(carNumL.l, carNumL.b + 20, 500, 40)];//(carNumL.left-20, carNumL.bottom+5, 200, 20)];
    ...
    if (days < 0) {
        ...
        if (iOS6) {
            NSMutableAttributedString *str = [[NSMutableAttributedString alloc] initWithString:[NSString stringWithFormat:@"距离年检 %d 天", days]];
            [str addAttribute:NSForegroundColorAttributeName value:COLOR_NAV range:NSMakeRange(5, [[NSString stringWithFormat:@"%d", days]length])];
            [timeL setAttributedText:str];
        }else{
            timeL.text = [NSString stringWithFormat:@"距离年检 %d 天", days];
        }
    }
