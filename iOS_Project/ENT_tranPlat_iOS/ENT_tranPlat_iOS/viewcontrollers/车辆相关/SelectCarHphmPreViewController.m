...
//    [self.view addSubview:scrollV];
    ...
    CGFloat width = (APP_PX_WIDTH - 0)/4;
    CGFloat height = 30*3;
    //[scrollV setContentSize:CGSizeMake(APP_WIDTH, height*8)];

//    UILabel *vlineL = [[UILabel alloc] initWithFrame:LGRectMake(5, APP_VIEW_Y/APP_PX_WIDTH, 2, height*8)];
//    [vlineL setBackgroundColor:COLOR_FRAME_LINE];
//    [self.view addSubview:vlineL];
    
    for (int i = 0; i < 31; i++) {
        NSString *key = [_keys objectAtIndex:i];
        SelectCarHphmPreView *v = [[SelectCarHphmPreView alloc] initWithFrame:LGRectMake(5 + width*(i%4), APP_VIEW_Y/PX_X_SCALE + height*(i/4), width, height) jiancheng:key quancheng:[_dataDict objectForKey:key]];
        v.delegate_ = self;
