...
- (void)viewDidLoad
{
    ...
    _segBView = [[SegmentButtonView alloc] initWithFrame:YYRectMake(APP_X, APP_VIEW_Y/PX_X_SCALE, APP_PX_WIDTH, 34/PX_X_SCALE) backgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"bg_white_top.png"]] andLineColor:COLOR_NAV];
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (![jdsbh isEqualToString:@""]) {
        ...
        cell.jdsbhL.text = [NSString stringWithFormat:@"%@**********%@",[jdsbh substringToIndex:3], [jdsbh substringFromIndex:(jdsbh.length - 2)]];
        
        //line
        [cell.bottomLine setFrame:YYRectMake(0, cell.bgImgView.h - 1, APP_PX_WIDTH, 1)];
    }else{
...
- (void)getDealRecordWithIndex:(NSInteger)index{
    ...
    NSDictionary *callBackDict = [NSDictionary dictionaryWithObjectsAndKeys:car, @"car", [NSString stringWithFormat:@"%d", index], @"index", nil];
    _isloading = YES;
        ...
        if (result == postSucc) {
            ...
            for (NSMutableDictionary *dict in orders) {
                ...
                for (NSString *hphm in _datasourceHphmArr) {
                    if (![car.hphm isEqualToString:hphm]) {
