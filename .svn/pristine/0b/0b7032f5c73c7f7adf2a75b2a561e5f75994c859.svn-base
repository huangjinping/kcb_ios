...
@property (nonatomic, retain)       NSString                *fdjhStr;
...
- (void)loadView_
{
    ...
    _mainBgView = [UIImageView backgroudTwoLineImageViewWithPXX:0 y:l.b + 20 width:APP_PX_WIDTH height:singleLineHeightPX*4];
    ...
    //_hphmTF.autocapitalizationType =   UITextAutocapitalizationTypeSentences;
    _hphmTF.autocapitalizationType = UITextAutocapitalizationTypeAllCharacters;
    // _hphmTF.keyboardAppearance = UIKeyboardTypeDefault;
    _hphmTF.keyboardType = UIKeyboardTypeASCIICapable;
    ...
    _sbdhTF.autocapitalizationType = UITextAutocapitalizationTypeAllCharacters;
    // _hphmTF.keyboardAppearance = UIKeyboardTypeDefault;
    _sbdhTF.keyboardType = UIKeyboardTypeASCIICapable;
    ...
    label.size = size;
    [_mainBgView addSubview:label];
    _fdjh = [UITextField mainTextFieldWithFrame:LGRectMake(label.r + 30, label.t, _mainBgView.w - label.r - 35 - 30 - 50, 40) placeholder:@"云 青 渝 琼 鲁五省可不填" tag:0 delegate:self];
    [_mainBgView addSubview:_fdjh];
    _fdjh.autocapitalizationType = UITextAutocapitalizationTypeAllCharacters;
    // _hphmTF.keyboardAppearance = UIKeyboardTypeDefault;
    _fdjh.keyboardType = UIKeyboardTypeASCIICapable;
...
- (void)submitButtonClicked:(UIButton*)button{
    ...
    if ([_hphmPrefixLabel.text isEqualToString:@"云"]||[_hphmPrefixLabel.text isEqualToString:@"青"]||[_hphmPrefixLabel.text isEqualToString:@"渝"]||[_hphmPrefixLabel.text isEqualToString:@"琼"]||[_hphmPrefixLabel.text isEqualToString:@"鲁"]) {
        ...
        if (![self changyongText]) {
            return;
        }else{
            [self changSubmitToBandcar];
...
-(BOOL)changyongText{
    ...
    if (!_carTypeL.text || [_carTypeL.text isEqualToString:@""]) {
    ...
    }else if (!self.fdjhStr || [self.fdjhStr isEqualToString:@""]){
        [UIAlertView alertTitle:@"信息提示" msg:@"发动机号不能为空"];
        return  NO;
        ...
        return  NO;
    }else if (self.fdjhStr.length < 6){
        [UIAlertView alertTitle:@"信息提示" msg:@"发动机号不小于6位"];
        return  NO;
...
- (BOOL)keepAndRuleText{
    ...
    if (!_fdjh.text || [_fdjh.text isEqualToString:@""]) {
        ...
        [UIAlertView alertTitle:@"信息提示" msg:@"发动机号应不小于6位"];
        return NO;
    }
...
#pragma mark-  常用车辆网络请求
- (void)changSubmitToBandcar{
   // [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    ...
    if (self.reBindHphm) {
        ...
        [dict setObject:@"adducar" forKey:@"action"];
    ...
    [[Network sharedNetwork] postBody:dict isResponseJson:NO doShowIndicator:YES callBackWithObj:nil onCompletion:^(PostResult result, id requestObj, NSObject *callBackObj) {
        ...
        if ([dic isKindOfClass:[NSDictionary class]]) {
            if([[dic valueForKey:@"code"]isEqualToString:@"0"]){
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息" message:[NSString stringWithFormat:@"常用车辆修改成功"] delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil, nil];
                ...
                NSNotificationCenter *center=[NSNotificationCenter defaultCenter];
                [center postNotificationName:@"zhuanshu" object:self userInfo:nil];
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息" message:[NSString stringWithFormat:@"绑定车辆失败:%@",[dic valueForKey:@"message"]] delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil, nil];
                ...
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示信息" message:[NSString stringWithFormat:@"绑定车辆失败"] delegate:self cancelButtonTitle:@"确认" otherButtonTitles:nil, nil];
                alert.tag = TAG_ALERT_BIND_FAILED;
                [alert show];
