...
- (id)initWithTarget:(UIViewController*)sender selector:(SEL)sel placehoders:(NSArray*)placehoderStrings{
    ...
    CGFloat y = 60*PX_Y_SCALE;
    CGRect frame = CGRectMake(space_x, APP_VIEW_Y + y, APP_WIDTH - space_x*2, height);
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        for (int i = 0; i <= [placehoderStrings count]; i++) {
            ...
            if (i == 0) {
                ...
                CopyUnableTextField *tf = [[CopyUnableTextField alloc] initWithFrame:rect];
                [self addSubview:tf];
                tf.placeholder = placeholder;
                tf.textAlignment = NSTextAlignmentLeft;
                tf.borderStyle = UITextBorderStyleLine;
                tf.delegate = self;
                [tf limitCHTextLength:8];
            }else{
                NSString *placeholder = [placehoderStrings objectAtIndex:i];
