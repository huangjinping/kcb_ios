...
@interface ChatBriefBlogViewController ()

@end

@implementation ChatBriefBlogViewController
            ...
            if ([matches count] > 0) {
                ...
                for (UIView *view = self.view.superview; view; view = [view superview]) {
                    ...
                    if ([nextResponder isKindOfClass:[UIViewController class]]) {
                        ...
                        [((UIViewController*)nextResponder).navigationController pushViewController:cwVC animated:YES];
                        break;
                    }
...
//        if ([nextResponder isKindOfClass:[UIViewController class]]) {
//            
//            ChatDetailViewController *cdVC = [[ChatDetailViewController alloc] init];
//            cdVC.titleName = @"详情";
//            ChatBlog *blog = [_blogs objectAtIndex:indexPath.row];
//            cdVC.blogs = [NSArray arrayWithObjects:blog, nil];
//            NSMutableArray *commonts = [[NSMutableArray alloc] initWithCapacity:0];
//            for (ChatComment *myComment in _comments) {
//                if ([myComment.bid isEqualToString:blog.id_]) {
//                    [commonts addObject:myComment];
//                }
//            }
//            cdVC.comments = commonts;
//            [((UIViewController*)nextResponder).navigationController pushViewController:cdVC animated:YES];
//            break;
//        }
//    }
...
- (void)egoRefreshTableFooterDidTriggerRefresh:(EGORefreshTableFooterView *)view{
    ...
    [self netGetAllBlogsWithIndexid:blog.id_ loadNew:NO bsort:self.blosort];
}
...
- (void)egoRefreshTableHeaderDidTriggerRefresh:(EGORefreshTableHeaderView *)view{
    ...
    [self netGetAllBlogsWithIndexid:nil loadNew:YES bsort:self.blosort];
}
...
- (void)loadView_{
    ...
    _blogTableView = [[UITableView alloc] initWithFrame:CGRectMake(APP_X, 0, APP_WIDTH, APP_HEIGHT - APP_NAV_HEIGHT - APP_TAB_HEIGHT-34)];
//    _blogTableView = [[UITableView alloc] initWithFrame:self.view.bounds];
    
    //_blogTableView = [[UITableView alloc] initWithFrame:CGRectMake(APP_X, APP_VIEW_Y, APP_WIDTH, APP_HEIGHT - APP_NAV_HEIGHT)];
//    _blogTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, APP_WIDTH, self.view.bounds.size.height)];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    if (APP_DELEGATE.shouldRefreshBlogList) {
        [self netGetAllBlogsWithIndexid:nil loadNew:YES bsort:self.blosort];
...
- (NSArray *)arrayWithChatCommentArray:(NSArray *)commentArray blog:(NSArray *)blogArray{
    ...
    for (ChatBlog *blog in blogArray) {
        ...
        if ([zanArr count] > 0) {
            ...
            string1 = [NSMutableString stringWithFormat:@"%@等%d人觉得很赞", zanUserString,[zanArr count]];
        }else{
            string1 = [NSMutableString stringWithFormat:@"动动手赞一个吧"];
        }
...
- (void)alertView:(UIAlertView *)alertView didDismissWithButtonIndex:(NSInteger)buttonIndex{
    ...
    if ([alertView.message isEqualToString:@"确认删除这条博文吗"]) {
        if (buttonIndex == 0) {
            ...
            [self netDeleteBlogs:[NSString stringWithFormat:@"%d",alertView.tag]];
        }
        ...
        if (buttonIndex == 0) {
            ...
            [self netDeleteComment:[NSString stringWithFormat:@"%d",alertView.tag]];
        }
...
//        NSLog(@"头像点击  row = %d",myBtn.tag-2000);
    ...
    for (UIView *view = self.view.superview; view; view = [view superview]) {
        ...
        if ([nextResponder isKindOfClass:[UIViewController class]]) {
            ...
            [((UIViewController*)nextResponder).navigationController pushViewController:chatBlogsVC animated:YES];
            break;
        }
...
- (void)blogZan:(UIButton*)button{
    ...
    [self netZanByUsername:APP_DELEGATE.userName andBlogId:[NSString stringWithFormat:@"%d",button.tag]];
}
...
- (void)blogPinglun:(UIButton*)button{
    ...
    _pingTextView = [[YFInputBar alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY([UIScreen mainScreen].bounds)-34, APP_WIDTH, 44)];
    
    _pingTextView.tagCount = button.tag;
    _pingTextView.pingOrHui = YES;
    _pingTextView.delegate = self;
    _pingTextView.isCarH = YES;
...
//    if (self.blosort) {
//        writeBlogVC.bsort = self.blosort;
//    }else{
//        writeBlogVC.bsort = @"0";
//    }
//    
...
- (void)chatBlogCell:(ChatBlogsCell *)cell clickedChatImage:(ChatImageView *)imgView
{
    ...
    for (UIView *view = self.view.superview; view; view = [view superview]) {
        ...
        if ([nextResponder isKindOfClass:[UIViewController class]]) {
            ...
            [((UIViewController*)nextResponder).navigationController pushViewController:browser animated:YES];
            break;
        }
...
- (void)pingLableClicked:(pingLable *)myLable{
    ...
    if ([userString length] < length) {
        _pingTextView = [[YFInputBar alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY([UIScreen mainScreen].bounds), 320, 44)];
...
- (void)netGetAllBlogsWithIndexid:(NSString*)indexId loadNew:(BOOL)loadNew bsort:(NSString*)bsort{
    ...
    //    _cellTestHeight = 0;
    [[ChatNetwork sharedChatNetwork] chatPostBody:bodyDict onUrl:@"findBlogs.do" withPostMethod:@"POST" isResponseJson:YES doShowIndicator:NO onView:nil callBackWithObj:isRefresh onCompletion:^(PostResult result, id requestObj, NSObject *callBackObj) {
        
        if(result == postSucc){
            
            APP_DELEGATE.shouldRefreshBlogList = NO;
            ...
            for (NSDictionary *dict in blogs) {
                ...
                blog.bsort = [dict analysisStrValueByKey:@"bsort"];
                if ([blog.bsort isEqualToString:@"10"]) {
                    continue;
                }
...
- (void)netPingByUsername:(NSString*)username andBlogId:(NSString*)blogId andContent:(NSString *)contents{
    ...
    [[ChatNetwork sharedChatNetwork] chatPostBody:bodyDict onUrl:@"addComment.do" withPostMethod:@"POST" isResponseJson:YES doShowIndicator:NO onView:self.view callBackWithObj:contents onCompletion:^(PostResult result, id requestObj, NSObject *callBackObj) {
        if (result == postSucc) {
...
{
    [[ChatNetwork sharedChatNetwork] chatPostBody:[NSMutableDictionary dictionaryWithObjectsAndKeys:blogId, @"blogid", nil] onUrl:@"reportBlog.do" withPostMethod:@"GET" isResponseJson:YES doShowIndicator:NO onView:nil callBackWithObj:nil onCompletion:^(PostResult result, id requestObj, NSObject *callBackObj) {
        if (result == postSucc) {
