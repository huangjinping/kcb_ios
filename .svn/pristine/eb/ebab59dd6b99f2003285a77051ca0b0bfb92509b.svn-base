...
- (void)configCellWithDic:(NSDictionary *)dic{
    ...
    if ([[dic[@"userName"] analysisConvertToString] isEqualToString:@"null"] || ![[dic[@"userName"] analysisConvertToString] isLegal]) {
        _phoneLabel.text = @"";
    }else{
        _phoneLabel.text = [[dic[@"userName"] analysisConvertToString]isLegal]?[dic[@"userName"] analysisConvertToString]:@"";
    }
}
