...
@interface BYPayInfoViewController ()
...
- (void)viewDidLoad {
    ...
    CGFloat space = 25;
    UIImageView *bgImgView = [UIImageView backgroudTwoLineImageViewWithPXX:0 y:lable.b + space + space +space  width:APP_PX_WIDTH height:contentLineHeight*6];
    [self.view addSubview:bgImgView];
    
    //    if (_isSucceed ==YES) {
    //
    //        lable = [[UILabel alloc] initWithFrame:LGRectMake(30, space, bgImgView.w - 30*2, space*3)];
    //        [lable convertNewLabelWithFont:FONT_NOMAL textColor:COLOR_FONT_INFO_SHOW textAlignment:NSTextAlignmentLeft];
    //        NSString *string=[NSString stringWithFormat:@"订单编号，消费密钥 :%@,%@",_orderNo,_DDInfo];
    //        lable.numberOfLines=0;
    //        [lable setText:string];
