...
- (void)configUI{
    ...
    [loginBtn addBorderWithWidth:1 color:kColor0X39B44A corner:3];
...
- (void)pwdBtn:(UIButton *)sender
{
    ...
    [_passwordFiled updateFocusIfNeeded];
    
    [_passwordFiled makeOffsetFromBeginning:_passwordFiled.text.length];
...
- (void)login{
    ...
    if (_accountFiled.text.length == 0) {
        msgError = @"请输入用户名";
    }else if(_passwordFiled.text.length == 0){
        msgError = @"请输入密码";
    }
    if (msgError) {
        [UIHelper showText:msgError ToView:self.view];
        return;
    }
    NSDictionary *param = @{@"account":_accountFiled.text,@"password":[_passwordFiled.text md5],@"vnum":@"1"};
