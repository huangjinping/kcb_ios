...
- (UITableView *)tableView{
    ...
    [_tableView registerClass:[OrderSelectCell class] forCellReuseIdentifier:selectCellId];
      //  [table reloadSections:[NSIndexSet indexSetWithIndex:indexPath.row] withRowAnimation:UITableViewRowAnimationNone];
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section{
    if (section == 3) {
        ...
        _totalLabel = [[UILabel alloc]initWithFrame:CGRectMake(view.width-40*x_6_plus-(view.width-_serviceLabel.width), 0, view.width-_serviceLabel.width, view.height)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_serFeeArr.count) {
        ...
        cell.price = [NSString stringWithFormat:@"%.2f",[_serFeeArr[indexPath.row][@"price"] floatValue]];
    }
